...
export default function App() {
  ...
  const generate = async () => {
    ...
    setError(""); setResult(null); setScreen("loading"); setStreakDone(false);
    ...
    try {
      ...
      if (!res.ok) { throw new Error("Fehler " + res.status + ": " + txt); }
      ...
      clearInterval(ticker.current);
      setError(e.message || "Unbekannter Fehler."); setScreen("form");
    }
  };
  ...
  const S = {
    ...
    dateChip: { display:"inline-block", background:"#f5f5f5", border:"1px solid #e0e0e0", borderRadius:8, padding:"8px 18px", fontSize:13, fontWeight:600, color:"#888", marginBottom:20 },
    ...
    opt: (a) => ({ padding:"9px 16px", borderRadius:8, border:`2px solid ${a?"#ff6666":"#e8e8e8"}`, background:a?"#fff0f0":"#fafafa", color:a?"#cc3333":"#555", fontSize:13, fontWeight:a?700:500, cursor:"pointer" }),
        ...
        {screen==="form" && (
          <div style={S.card}>
            ...
            <div style={S.qWrap}>
              ...
              <div style={S.optRow}>{BERUF_OPTIONS.map(o=><div key={o} style={S.opt(answers.beruf===o)} onClick={()=>setA("beruf",o)}>{o}</div>)}</div>
              {answers.beruf==="Anderes" && <textarea style={{...S.ta,marginTop:8,minHeight:44}} placeholder="Was machst du genau?" value={answers.berufCustom} onChange={e=>setA("berufCustom",e.target.value)} rows={1}/>}
            </div>
            <div style={S.qWrap}>
              <label style={S.qLabel}><span style={S.qNum}>2</span>Wo stehst du gerade?</label>
              <div style={S.optRow}>{LEVEL_OPTIONS.map(o=><div key={o} style={S.opt(answers.level===o)} onClick={()=>setA("level",o)}>{o}</div>)}</div>
            </div>
            <div style={S.qWrap}>
              <label style={S.qLabel}><span style={S.qNum}>3</span>Was ist heute dein Fokus?</label>
              <div style={S.optRow}>{FOKUS_OPTIONS.map(o=><div key={o} style={S.opt(answers.fokus===o)} onClick={()=>setA("fokus",o)}>{o}</div>)}</div>
            </div>
            {error && <div style={S.errBox}>⚠ {error}</div>}
            <button style={S.btnMain} onClick={generate}>Meinen Tagesplan erstellen →</button>
          </div>
        )}
        {screen==="loading" && <div style={S.loadWrap}><div style={S.spinner}/><div style={S.ltxt}>{loadMsg}</div></div>}
        {screen==="result" && result && (
          <div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
              {[answers.beruf==="Anderes"?answers.berufCustom:answers.beruf,answers.level,answers.fokus].filter(Boolean).map((c,i)=>(<span key={i} style={S.chip}>{c}</span>))}
            </div>
            <div style={S.rcrd}>
              ...
              <div style={S.rbody}>
                ...
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:11,fontWeight:800,letterSpacing:1.5,textTransform:"uppercase",color:"#cc3333",marginBottom:6}}>Post-Ready Hook</div>
                  <div style={S.hookBox}>
                    <button style={{background:"#3d3d3d",border:"none",color:"#fff",borderRadius:5,padding:"3px 9px",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"'Montserrat',sans-serif",float:"right",marginLeft:8}} onClick={()=>doCopy("hook",result.content?.hook||"")}>{copied["hook"]?"Kopiert ✓":"Kopieren"}</button>
                    {result.content?.hook}
                  </div>
                </div>
                <div style={{marginBottom:8}}><strong style={{color:"#999",fontSize:11,textTransform:"uppercase",letterSpacing:1}}>Inhalt </strong><br/><span style={{color:"#555"}}>{result.content?.inhalt}</span></div>
                <div><strong style={{color:"#cc3333",fontSize:11,textTransform:"uppercase",letterSpacing:1}}>CTA </strong><br/><span style={{color:"#555"}}>{result.content?.cta}</span></div>
              </div>
            </div>
            <div style={S.rcrd}>
            ...
            <div style={S.rcrd}>
              ...
              <div style={S.rbody}>
                <div style={S.todoBox}><strong style={{color:"#5a3a8a"}}>{result.daily_todo?.aufgabe}</strong></div>
